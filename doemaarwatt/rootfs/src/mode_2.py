...
async def mode_2_loop():
    print(f'Mode 2 (manual mode) initializing')

    dm_cfg = config.get_data_manager_config()
    dm_cfg['name'] = DM

    inv_phase_map = config.get_inverter_phase_map()
    print(f'manual mode: the following inverters are enabled for each phase:')
    ...
    print(f'manual mode: charging configuration for each phase:')
    ...
    while True:
        ...
        try:
            print(f'manual mode: connecting and assuming control')
            ...
            print(f'manual mode: encountered an error: {e}')
